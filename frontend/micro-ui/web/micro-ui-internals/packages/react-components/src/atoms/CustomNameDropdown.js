...
const CustomNameDropdown = (props) => {
      ...
      {dropdownStatus ? (
        props.optionKey ? (
          ...
          >
            {props.optionsHeader && (
              <div
                className="profile-dropdown--item display: flex"
                style={{
              ...
              >
                {Object.keys(props.optionsHeader).map((key, i) => (
                  <span key={i} style={{ flex: 1, padding: "8px" }}>
                    {props.t ? props.t(props.optionsHeader[key]) : props.optionsHeader[key]}
                  </span>
                ))}
              </div>
            )}
                ...
                return (
                  ...
                  >
                    ...
                    {props.optionsHeader ? (
                      Object.keys(props.optionsHeader).map((key, i) => (
                        <span key={i} style={{ flex: 1, padding: "8px" }}>
                          {props.t ? props.t(option[key]) : option[key]}
                        </span>
                      ))
...
CustomNameDropdown.defaultProps = {
  customSelector: null,
  showArrow: true,
};

export default CustomNameDropdown;
